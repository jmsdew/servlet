...
public class QueryStringTestServlet extends HttpServlet {
    ...
    * 톰캣 서블릿 컨테이너가 요청 url로 매핑된 servlet 클래스의 인스턴스를 생성하여 service method 를 호출하고
    * httpServlet 을 상속 받아 오버라이딩한 현재 클래스의 doGet() method가 동적 바인딩에 의해 호출된다.
    * =============================================================================================
    * Service로 부터 전달받은 httpServletRequest는 요청 시 전달한 값을 getParameter() Method로 추출할 수 있다.
    * 이 때 인자로 input 태그에 지정한 name 속성의 값을 문자열 형태로 전달해 주면 된다.
    * 화면에서 전달한 form 내의 모든 input 태그의 값들을 hashMap 으로 관리하고 있으므로,
